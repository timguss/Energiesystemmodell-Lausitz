...
void addOrUpdateClient(const String &name, const String &ip){
  for(int i=0;i<MAX_CLIENTS;i++){
    if(clients[i].name == name){
      clients[i].ip = ip; clients[i].lastSeen = millis();
  ...
  for(int i=0;i<MAX_CLIENTS;i++){
    if(clients[i].name.length()==0){
      clients[i].name = name; clients[i].ip = ip; clients[i].lastSeen = millis();
...
// Robust extractor (handles quoted strings, JSON objects/arrays, unquoted tokens)
String extractValueFromBody(const String &b, const char *key){
  String search = String("\"") + String(key) + String("\"");
  int idx = b.indexOf(search);
  if(idx < 0) return "";
...
void handleForward(){
  ...
  String target = extractValueFromBody(b, "target");
  String method = extractValueFromBody(b, "method");
  String path = extractValueFromBody(b, "path");
  String body = extractValueFromBody(b, "body");
  ...
  if(targetIp.length()==0){ Serial.printf("[FEHLER] Target '%s' nicht registriert\n", target.c_str()); server.send(404,"text/plain","target not registered"); return; }

  String url = "http://"+targetIp+path;
  Serial.printf("[FORWARD] %s zu %s (%s)\n", method.c_str(), target.c_str(), url.c_str());

  // Wenn body ein JSON-Objekt/Array beginnt, normalisiere escaped sequences -> reale Steuerzeichen
  bool bodyIsJson = body.length()>0 && (body.charAt(0) == '{' || body.charAt(0) == '[');
  if(bodyIsJson){
    // convert literal "\\r\\n" -> real "\r\n" and similar
    // note: replace() mutates String in-place
    body.replace("\\\\r\\\\n", "\r\n");
    body.replace("\\\\n", "\n");
    body.replace("\\\\r", "\r");
    body.replace("\\\\t", "\t");
