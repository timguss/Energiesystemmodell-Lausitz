...
function initScrollAnimations() {
  ...
  const archElements = document.querySelectorAll(
    ".architecture-text, .architecture-visual",
  );
...
async function loadSensors() {
  try {
    ...
    if (response.ok) {
      sensorConfig = await response.json();
      for (const [id, sensor] of Object.entries(sensorConfig)) {
        if (sensor.target_card) {
          const container = document.getElementById(
            `sensors-${sensor.target_card}`,
          );
          if (container) {
...
async function refreshSensors() {
  const devices = [
    ...new Set(Object.values(sensorConfig).map((s) => s.device)),
  ];
      ...
      if (response.ok) {
        ...
        for (const [id, sensor] of Object.entries(sensorConfig)) {
          if (sensor.device === device) {
            const el = document.getElementById(`sensor-display-${id}`);
            if (el) {
              let val = state;
    ...
    if (data.scenarios && Object.keys(data.scenarios).length > 0) {
      for (const [key, scenario] of Object.entries(data.scenarios)) {
        ...
        if (scenario.target_card) {
          const cardEl = document.getElementById(
            `scenarios-${scenario.target_card}`,
          );
      ...
      if (mainContainer)
        mainContainer.innerHTML =
          '<div class="loading-spinner">Keine Szenarien verfügbar</div>';
    }
  } catch (error) {
    console.error("Error loading scenarios:", error);
    if (mainContainer)
      mainContainer.innerHTML =
        '<div class="loading-spinner">Fehler beim Laden</div>';
  }
}
...
async function executeScenario(scenarioName, state, stateName, buttonElement) {
  ...
  try {
    ...
    if (result.success || result.host_response) {
      ...
      showNotification(
        `Szenario "${stateName}" erfolgreich ausgeführt`,
        "success",
      );
...
async function loadRelays() {
  ...
  // Provide a final cleanup pass for any containers that received 0 relays
  containers.forEach((id) => {
    const el = document.getElementById(id);
    if (el) {
  ...
  try {
    ...
    if (meta && meta.names) {
      for (let i = 0; i < meta.count; i++) {
        ...
        // Use Mapping
        const targetId =
          RELAY_MAPPING.esp3 && RELAY_MAPPING.esp3[i]
            ? RELAY_MAPPING.esp3[i]
            : "reserve-controls";
        const targetContainer = document.getElementById(targetId);
        if (targetContainer) {
          const spinner = targetContainer.querySelector(".loading-spinner");
  ...
  try {
    ...
    if (meta && meta.names) {
      for (let i = 0; i < meta.count; i++) {
        ...
        // Use Mapping
        const targetId =
          RELAY_MAPPING.esp4 && RELAY_MAPPING.esp4[i]
            ? RELAY_MAPPING.esp4[i]
            : "reserve-controls";
        const targetContainer = document.getElementById(targetId);
        if (targetContainer) {
          const spinner = targetContainer.querySelector(".loading-spinner");
...
function createRelayItem(globalIndex, name, state, isOffline = false) {
  ...
  if (!isOffline) {
    input.onchange = () =>
      toggleRelay(globalIndex, input.checked ? 1 : 0, input);
  }
...
async function setWind(val) {
  try {
    ...
    showNotification(
      `Wind ${val ? "eingeschaltet" : "ausgeschaltet"}`,
      "success",
    );
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (tempEl && state.temp !== undefined) {
        tempEl.textContent =
          state.temp === null ? "--°C" : `${Number(state.temp).toFixed(1)}°C`;
      }
    }
  } catch (error) {
