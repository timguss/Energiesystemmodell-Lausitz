...
function mkRelayRow(global_idx, name, state) {
  ...
  label.innerHTML =
    "<strong>" + name + '</strong><div class="small">#' + global_idx + "</div>";
    ...
    if (j.scenarios && Object.keys(j.scenarios).length > 0) {
      ...
      for (const [scenarioKey, scenarioData] of Object.entries(j.scenarios)) {
        ...
        scenarioData.states.forEach((stateInfo) => {
          ...
          btn.onclick = () =>
            executeScenario(scenarioKey, stateInfo.id, stateInfo.name, btn);
      ...
      container.innerHTML =
        '<div class="small">Keine Szenarien verfügbar</div>';
    }
  } catch (e) {
    console.error("scenarios err", e);
    document.getElementById("scenarios").innerHTML =
      '<div class="error">Fehler beim Laden</div>';
  }
}
...
async function refreshEsp3() {
  try {
    ...
    let j = await r.json();
    let body = j.body || j;
    document.getElementById("windState").textContent = body.running
      ? "AN"
      : "AUS";
